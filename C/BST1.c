...
bool Search(PNODE Head, int iNo)
{
		if(Head != NULL)
		{
			if(iNo > Head->data)        // Data is greater
			{
				Head = Head->rchild;
				Search(Head, iNo);			
			}
			else if(iNo < Head->data)      // Data is smaller
			{
				Head = Head->lchild;
				Search(Head, iNo);
			}
			else if(iNo == Head->data)    // Data is same
			{
				return true;
			}
			else
